    ...
    <el-card>
      ...
      <el-table :data="orderList"
        ...
        <el-table-column label="下单时间"
                         prop="create_time">
          <template v-slot="scope">{{scope.row.create_time | dataFormat}}</template></el-table-column>
        <el-table-column label="操作">
          <el-button type="primary"
                     size="mini"
                     icon="el-icon-edit"
                     circle
                     @click="editAddr"></el-button>
  ...
  data () {
    return {
      ...
      addressFormRules: {
        ...
        address2: [
          { required: true, message: '请输入具体地址称', trigger: 'blur' }
        ]
      },
      cityData,
      // 物流信息
